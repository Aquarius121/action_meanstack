...
var monthly_unique_users_graph = (function() {

    var chart_template_def = '<div class="chart-title">Monthly Unique Users</div>' +
    ...
    function init(container, options) {
        var min_date = moment(options.to).subtract('months', 3);
        min_date.set('date', 1);
        min_date.set('hour', 0);
        ...
        console.log('querying for unique users from ' +
            moment(start_date).format('MMM DD') + ' to ' +
            moment(options.to).format('MMM DD'));
        ...
        $.ajax({
            ...
            // compute tick positions (month starts)
            ...
            while(tickTimeIterator.valueOf() < options.to) {
                ...
                tickTimeIterator = tickTimeIterator.month(tickTimeIterator.month() + 1);
            ...
            container.find('.graph-container').highcharts({
                ...
                tooltip: {
                    formatter: function() {
                        return '<b>' + moment(this.x + 1).format('MMMM') + '</b>: ' + this.y + ' unique user' + (this.y != 1 ? 's' : '');
                    }
                },
                xAxis: {
                    min: start_date,
    ...
    function _dataHandler(data, options) {
        ...
        data.forEach(function(month_record) {
            month_record.values.forEach(function(platform_record) {
                //platform, count
                if(typeof(values[platform_record.platform]) == 'undefined') {
                    values[platform_record.platform] = [];
                }
                values[platform_record.platform].push([month_record.to_time, platform_record.count]);
            });
        });

        var series = [];
        Object.keys(values).forEach(function(inner_key) {
            // TODO: for each inner_key, we need to make sure each month in the range has a record (even if we must add a 0)
    ...
    function _csvHandler(data, options) {
        ...
        var csv_contents = '"Month End Date"';
        ...
        Object.keys(day_values).forEach(function(day_key) {
            csv_contents += '"' + moment(parseInt(day_key)).format('MM/DD') + '"';
